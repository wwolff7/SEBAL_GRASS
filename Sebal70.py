...
ETo = float(input('Place the diary value of reference evapotranspiration (EToi) from the weather station - (mm): '))
...
for MTLfile in glob.glob("LC*.txt"):
    break

for line in open(MTLfile):
...
Ts_mean=g.parse_command("r.univar", flags="g", map="Ts", quiet = True)["mean"]

print "Mean surface temperature:", Ts_mean,"K"
